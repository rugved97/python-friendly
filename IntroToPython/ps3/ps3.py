...
def get_word_score(word, n):
    ...
    first_component = 0
    for char in word:
        first_component += SCRABBLE_LETTER_VALUES[str.lower(char)]
...
def update_hand(hand, word):
    ...
    for char in word:
        ...
        if lower_char in new_hand.keys():
            if new_hand[lower_char] != 0:
                new_hand[lower_char] -= 1
...
def is_valid_word(word, hand, word_list):
    ...
    if idx != -1:
        for vowel in VOWELS:
            ...
            temp_word[:0] = word
            temp_word[idx] = vowel
            if ''.join(temp_word).lower() in word_list:
                possible_words.append(''.join(temp_word).lower())
    ...
    for char in word:
        lower_char = char.lower()
        if lower_char in temp_hand.keys():
            if temp_hand[lower_char] != 0:
                temp_hand[lower_char] -= 1
            else:
                return False
        else:
            return False
...
def substitute_hand(hand, letter):
    ...
    """
    temp_hand = hand.copy()
    if letter.lower() not in temp_hand.keys():
        return temp_hand

    value = temp_hand[letter.lower()]
    new_letter = random.choice(VOWELS + CONSONANTS)
    while new_letter in temp_hand.keys():
        new_letter = random.choice(VOWELS + CONSONANTS)

    temp_hand[new_letter] = value
    ...
    while no_of_hands > 0:
        ...
        if substitute:
            ...
            if substitute_input.lower() == 'yes':
                undesired_letter = input('Enter desired letter: ')
                hand = substitute_hand(hand, undesired_letter)
        ...
        print('Score for this hand', hand_score)
        no_of_hands -= 1

    print('Score of the series', series_score)
    # handOrig = {'a': 1, 'c': 1, 'i': 1, 'f': 1, '*': 1, 't': 1, 'x': 1}
    # play_hand(handOrig, word_list)
